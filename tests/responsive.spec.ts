...
test.describe('responsive overflow', () => {
  ...
  test('professional theme keeps its cyan identity and profile content on mobile', async ({
    ...
    for (const path of ['/profesional', '/es/profesional']) {
      ...
      await expect(page.locator('[data-certificate]')).toHaveCount(4);
      await expect(page.locator('.certificate-preview-image')).toHaveCount(4);
      await expect(page.locator('.certificate-link')).toHaveCount(4);
